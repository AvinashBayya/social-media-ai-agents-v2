...
| Self-hosted model inference                                  | No GPU quota                   | Needs a GPU host. The Azure quota route died with the subscription (removed 2026-08-20) and was never submitted. GPU compute is not free, which always contradicted the zero-budget rule; `llm.ts` keeps using hosted free tiers until a box exists.                                                                                                                                                                   |
...
Git Bash rewrites any argument starting with `/` into a Windows path; `export MSYS_NO_PATHCONV=1` stops that, or every
